...
ALTER TABLE [Goods] ADD [id_type_provider] INT;

ALTER TABLE [Goods]
ADD CONSTRAINT [FK_Provider_id_type] FOREIGN KEY ([id_type_provider]) REFERENCES [Type_Provider]([id_type_provider]);
...
--4--
GO
create  procedure [GetGoods] (@tp_type_provider INT = NULL)
AS BEGIN
SELECT g.[g_naiming], t.[tp_type_provider] FROM [Goods] g, [Type_Provider] t  WHERE g.[id_type_provider] = t.[id_type_provider] AND t.[tp_type_provider] = N'Федеральный' OR t.[tp_type_provider] = N'Областной';
END
exec [GetGoods];
...
SELECT * FROM [Hardware]
WHERE [h_service_end_date] NOT BETWEEN @h_service_start_date and @h_service_end_date;
END;
exec [WeekRepair]
...
END;
exec [RepairEndDate] @id_bardware = 8, @b_repair_end_date = '2023-1-21';
...
END
exec [AddDeliver] 10, N'2023-2-18', 10, 1, 4;
select * from [Deliveries];
select * from [Goods];
...
END;
EXEC [SolуPerDay] @sale_date = '2023-1-1';
